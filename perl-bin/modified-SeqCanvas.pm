# $Id: modified-SeqCanvas.pm,v 1.1.1.1 2007/05/03 16:15:58 matthew_links Exp $ 
=head2 NAME

asio::Tk::SeqCanvas.pm - Graphical display of SeqI compliant objects
...
# also be triggered...
	...
	my $sV = $self->ScrollFrame->Scrollbar('-orient' => 'vertical', '-command' => sub {	
												$self->FinishedLabelCanvas->yview(@_); 
												$self->DraftLabelCanvas->yview(@_);
												$self->FinishedCanvas->yview(@_); 
												$self->DraftCanvas->yview(@_);
	});
    	...
    	$self->DraftCanvas->configure('-yscrollcommand' => ['set' => $sV] ); # since they are identical only one canvas needs to feed-back to the scroll bar to show it's extents
     	$s->pack('-side'=>'bottom', '-fill'=>'x', '-expand' => 'x');
     	$sV->pack('-side'=>'left', '-fill'=>'y', '-expand' => 'y');
        $self->ScrollBar($s);
        $self->ScrollBar($sV);
    } else {			# vertical
