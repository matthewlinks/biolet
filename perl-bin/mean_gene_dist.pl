...
EOF
			...
			if ($seq_len > 0){
				...
				die "$gb_file $acc\nError don't have a clue how to figure out the length of this sequence because the NCBI is hiding it!!!!";
			}
		}
	}
	...
	print "There were $total_count genes with a mean of ",sprintf("%0.2f",$floating_mean/$total_count),"\n";
...
#	@gff1_feats = sort sort_feats_by_start @gff1_feats;
	...
	print "\tLoaded all features from $gff_file2 into memory\n";
