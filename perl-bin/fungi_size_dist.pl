...
print "Organism\tTYPE\tCOUNT\tMEAN\n";
...
#		print "$f\t";
		...
		comparative_genomics($acc,$acc2waveGff->{$acc},$gb_file,$seq_len,$f);
#		print "\n";
	}
}
...
sub comparative_genomics{
	...
	my ($gff1_count,$gff1_mean) = average_feat(\@gff1_feats);
...
#	}
	...
	my ($gff2_count,$gff2_mean) = average_feat(\@gff2_feats);
	print "$f\t$acc\tGENE\t$gff2_count\t$gff2_mean\n";
	print "$f\t$acc\tWAVE\t$gff1_count\t$gff1_mean\n";

	# what is the average intergenic distance 
#	my $mean_dist = $seq_len / $count_possible;
#	print "There should be 1 feature every ",sprintf("%0.2f",$mean_dist),"\n";
...
sub average_feat{
	...
	my $total_length = 0;
	foreach my $f (@{$feats}){
		my $len = abs($f->start() - $f->end());
		$total_length += $len;
		++$count;
	}
	my $mean = int($total_length/$count);
