...
sub dump_feats_by_linkage {
	...
	print "Organism\tLinkageGroup\tLinkageGroupLength\tNumFeatures\tTotalFeatureLength\tMeanFeatureLength\tLengthFeatureDiscrete\tPrecentiFeatureDiscrete\tLengthRetrogradeFeature\tPercentRetrogradeFeature\n";
	...
	foreach my $gb_file (@{$gb_gffs}){
		...
		my ($bases_coding,$bases_coding_total,$bases_retro,$bases_extra_coding) = (0);

		my $count_1=0;
		my $count_2=0;

		for(my $i = 1;$i<=$seq_len;$i++){
				my ($strand1,$strand2) = split(/\./,$feat_types{'coding'}->[$i]);
				++$bases_retro if ($strand1 > 0)and($strand2 >= 0.00001);
				++$bases_coding if ($strand1 > 0)or($strand2 >= 0.00001);
				++$count_2 if ($strand2 >= 0.00001);
				++$count_1 if ($strand1 > 0);
		}
		
		print "$bases_coding\t",($bases_coding/$seq_len)*100,"\t","$bases_retro\t",($bases_retro/$seq_len)*100,"\t\n";
		undef %feat_types;
	}
}
...
#			($file =~ /\-$acc\.\d+\-[\w\d\_]+\-\d+\-\d\.nuc$/)	# NCBI can add some sort of xref into the file name
	...
	if (!(@files)){
		die "\tFor some reason there were no files found for $acc in $dir $frag_dir\n" if ($DEBUG);
	}

	my $asd = @files;
	warn "here1 $asd $dir $frag_dir $acc";

#	print "$acc\n";
	foreach my $file (@files){
		warn "Looking at $file for length data";
#		print "\t$file\n";
		++$nfiles;
