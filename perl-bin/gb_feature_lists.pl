...
foreach my $org (@org_list){
	my $dir = $root . '/' . $org;
	feats_by_linkage_group($dir);
#	feats_by_organism($dir);
}
...
sub feats_by_organism {
	...
	foreach my $gb_file (@{$gb_gffs}){
		...
		warn "\tParsing linkage group $gb_file ($acc)" if $DEBUG;
		if ($accs_2_skip{$acc}){
		...
		# read in all the features for this linkage group
		comparative_genomics($gb_file,\%feat_types);
	}	
	print "Total for $dir\n";
	foreach my $type (sort {$a cmp $b} keys %feat_types){
		print "$type\n";
		foreach my $e ( sort {$a <=> $b} @{$feat_types{$type}}){
			print "\t$e\n";
		}
	}
}

sub feats_by_linkage_group {
	my $dir = shift;
	print "Looking at $dir\n";
...
sub comparative_genomics{
	...
	while(my $f2 = $gff2io->next_feature()){
		if ($ignored_feats{$f2->primary_tag()} eq $feat_type) {
			my $len = ($f2->end() - $f2->start()) +1;
			push(@{$feat_types->$feat_type},$len);
		}
	}
	$gff2io->close();
}
