...
# header
print "ORGANISM\t";
foreach my $type (sort {$a cmp $b} keys %{$seen_feats{types}}){
	print "$type count \t$type mean\t";
}
print "\n";

foreach my $o (sort {$a cmp $b} keys %{$seen_feats{orgs}}){
	print "$o\t";
	# simple means
	foreach my $type (sort {$a cmp $b} keys %{$seen_feats{types}} ){
		if ($seen_feats{orgs}->{$o}->{$type}->{count}){
			my $mean = sprintf("%0.0f",$seen_feats{orgs}->{$o}->{$type}->{total_len} / 
				$seen_feats{orgs}->{$o}->{$type}->{count});
			print $seen_feats{orgs}->{$o}->{$type}->{count},"\t$mean\t";
		}else{
			print "0\t0\t";
		}
	}
...
sub handle_dir {
	...
	$seen_feats{orgs}->{$name} = \%feat_types;
}
...
sub comparative_genomics{
	...
	while(my $f2 = $gff2io->next_feature()){
		if (!($ignored_feats{$f2->primary_tag()})){
			++$seen_feats{types}->{$f2->primary_tag};
			$feat_types->{$f2->primary_tag}->{total_len} += ($f2->end() - $f2->start()) +1;
