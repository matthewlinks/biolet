...
sub handle_dir {
	...
	foreach my $gb_file (@{$gb_gffs}){
		...
		if ($seq_len > 0){
			...
			# this can arise because NCBI does NOT maintain the accession in the filename for the ganbank flat file
			# no idea why this can come up but it does
			# first seen in Apis mellifera
			if ($DEBUG) { warn <<EOF;
	Could not figure out sequence length for $acc
EOF
			}
			open(GFF,"<$gb_file") or die "Error opening $gb_file for reading: $!";
			my $line = <GFF>;
			chomp $line;
			undef $acc;
			($acc) = split(/\t/,$line);
			die "Error could not get first accession from gff $gb_file ($line,$acc)" unless defined $acc;
			$seq_len = get_seq_length($acc,$dir);
			if ($seq_len > 0){
				my ($mean,$count) = comparative_genomics($acc,$acc2waveGff->{$acc},$gb_file,$seq_len);
				push(@counts,$count);
				push(@means,$mean);
			}else{
				die "Error don't have a clue how to figure out the length of this sequence because the NCBI is hiding it!!!!";
			}
		}
	}

	# simple mean across all chromosomes 
	my $total_count = 0;
	my $floating_mean = 0;
	...
	foreach my $k (keys %allowed_feats){
		print "\t$k\n";
	}
	print "There were $total_count genes with a mean length of ",sprintf("%0.2f",$floating_mean/$total_count),"\n";
...
sub comparative_genomics{
	...
	while(my $f2 = $gff2io->next_feature()){
		if ($allowed_feats{$f2->primary_tag()}){
			$total_len += ($f2->end() - $f2->start()) +1;
			++$count_possible;
		}else{
			++$ignored_feats{$f2->primary_tag()};
		}
	}
	print "Examined all features from $gff_file2\n";
	$gff2io->close();
	if ($count_possible){	
		# what is the average intergenic distance 
		my $mean_length = $total_len / $count_possible;
		return (sprintf("%0.2f",$mean_length),$count_possible);
	}else{
		print "There was a problem ($gff_file2,$count_possible)\n";
		return (0,$count_possible);
	}
