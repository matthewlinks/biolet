...
sub dump_feats_by_linkage {
	...
	print "Organism\tLinkageGroup\tLinkageGroupLength\tNumFeatures\tTotalFeatureLength\tMeanFeatureLength\tLengthFeatureDiscrete\tPrecentiFeatureDiscrete\tLengthRetrogradeFeature\tPercentRetrogradeFeature\tAbsRetro\tPercentAbsRetro\tBasesExtraRetro\n";
	...
	foreach my $gb_file (@{$gb_gffs}){
		...
		my ($acc,$path,$suffix) = fileparse($gb_file,$gb_gff_suffix);
		warn "\tLooking at $gb_file ($acc)" if $DEBUG;
		if ($accs_2_skip{$acc}){
			warn "Skipping $gb_file becasuse it is on the do not touch list ($acc)";
			next;
		}

                if ($acc =~ /\_alt\_chr/){
                        warn "Skipping $gb_file becasue it is an alternate chromosome\n";
                        next;
                }
		warn "seq = $seq_len";
		comparative_genomics($gb_file,\%feat_types,$seq_len);


		my $org = basename($dir);

		# simple dump
		print "$org\t$acc\t$seq_len\t",$feat_counts{$magic_type},"\t",$feat_types{'total_length'},"\t",($feat_types{'total_length'}/$feat_counts{$magic_type}),"\t";
		my ($bases_coding,$bases_coding_total,$bases_retro,$bases_extra_coding) = (0,0,0,0);

		my $abs_retro = 0;
		my $bases_extra_retro =0;
		
		for(my $i = 1;$i<=$seq_len;$i++){
				my ($strand1,$strand2) = ($feat_types{'coding'}->[$i]->{sense},$feat_types{coding}->[$i]->{antisense});
				++$bases_retro if ($strand1 > 0)and($strand2 > 0);
				++$bases_coding if ($strand1 > 0)or($strand2 > 0);
				$abs_retro += ($strand1 + $strand2) if (($strand1 + $strand2) >1);
				$bases_extra_retro++ if (($strand1+$strand2)>2);

		}
		
		print "$bases_coding\t",($bases_coding/$seq_len)*100,"\t","$bases_retro\t",($bases_retro/$seq_len)*100,"\t",$abs_retro,"\t",($abs_retro/$seq_len)*100,"\t$bases_extra_retro\t","\n";
		undef %feat_types;
		undef %feat_counts;
	}
}
...
sub comparative_genomics{
	...
	while(my $f2 = $gff2io->next_feature()){
		my $feature_type = $f2->primary_tag();
		
		$feat_counts{$feature_type}++;

		if ($feature_type eq $magic_type){
			my $len = ($f2->end() - $f2->start()) +1;
			$feat_types->{'total_length'}+=$len;
			# count the number of times that the nucletide is used to encode a protein
			# could indicate gene models but not sure
#			for(my $i = $f2->start();$i<=$f2->end();++$i){
#				$feat_types->{'coding'}->[$i]++;
#			}
			# mark the retrograde encoding
			if ($f2->strand() == 1){
				for(my $i = $f2->start();$i<=$f2->end();++$i){
					$feat_types->{'coding'}->[$i]->{sense} += 1;
				}
			}elsif($f2->strand() == -1){
				die "Error end is less than start" if ($f2->end < $f2->start);
#				warn "$linkage_len,",$f2->start,",",$f2->end,"\n";
				my $e = ($linkage_len - $f2->start) +1;
				my $s = ($linkage_len - $f2->end) +1; 
				for(my $i = $s;$i<=$e;++$i){
					# Possible overflow 
					$feat_types->{'coding'}->[$i]->{antisense} += 1;
				}
			}
		}
	}
